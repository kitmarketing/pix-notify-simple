...
serve(async (req) => {
  try {
    ...
    // 🔑 TXID correto: prioriza txid do banco, depois endToEndId, depois UUID aleatório
    const txidFinal = evento.txid && evento.txid !== "sem-txid"
      ? evento.txid
      : (evento.endToEndId && evento.endToEndId !== "sem-txid"
          ? evento.endToEndId
          : crypto.randomUUID());

    const valor = parseFloat(evento.valor ?? "0") || 0;
